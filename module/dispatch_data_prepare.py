...
def redefine_col_name(ps_loc_data, taxi_loc_data): 
    # 컬럼명 변경
    ps_loc_data.columns = ['no', 'cartype', 'ps_type', 'ps_loc_0', 'ps_loc_1', 'adm_cn_start', 'adm_cn_end','call_time', 'set_time', 'ps_route', 'ps_timestamp', 'ps_distance']
    taxi_loc_data.columns = ['no', 'cartype', 'work_start', 'work_end', 'board_status', 'tx_loc', 'adm_cd']
    # 전처리 후 필요한 컬럼만 추출
    ps_loc_data = ps_loc_data[['ps_type', 'ps_loc_0', 'ps_loc_1', 'call_time', "adm_cn_start", "adm_cn_end", 'ps_route', 'ps_timestamp', 'ps_distance']]
    taxi_loc_data = taxi_loc_data[['no', 'cartype', 'work_start', 'work_end', 'board_status', 'tx_loc', 'adm_cd']]
    #콜 잡고 차량 올때까지 대기시간

...
###### Page1
# 1-a
page1_1a = f"전체 호출 수 - 총 {len(passenger_locations)}건, 실패 호출 수 - 총 {len(all_fail_data)}건, 호출 실패율 - {round(len(all_fail_data)/len(passenger_locations)*100,2)}%"
...
all_fail_data.columns = ["geometry", "fail_time"]
...
call_passenger_distribution = pd.DataFrame(pd.cut(passenger_locations["call_time"], bins = bins, right=False, labels = labels).value_counts(sort=False)).reset_index()
...
page1_1b = go.Figure()
page1_1b.add_trace(go.Scatter(x=call_passenger_distribution["call_time"][1:19], y=call_passenger_distribution["cnt"][1:19],
                         mode="lines+markers", 
                         name="콜 승객"))
page1_1b.add_trace(go.Scatter(x=fail_passenger_distribution["fail_time"][1:19], y=fail_passenger_distribution["cnt"][1:19],
                         mode="lines+markers",
                         name="콜 실패 승객"))
page1_1b.update_layout(
    xaxis = dict(
        tickmode = 'array',
        tickvals = [i for i in range(7,25)],
        ticktext = [f"{i}시" for i in range(7,25)]
    )
)
page1_1b.update_xaxes(range=[6.5, 24.5])
...
waiting_time = waiting_time.loc[waiting_time["time"] <= 1500]
...
page1_2b.update_layout(
    ...
    yaxis = dict(
        tickmode = 'array',
        tickvals = [i for i in range(20,121, 20)]),
...
###### Page2
# 1
empty_tx_per = [round(i/(i+j), 2) for i,j in zip(empty_tx, drive_tx)][60:1081]
drive_tx_per = [1-i for i in empty_tx_per]
t = [i for i in range(360, 1801)][60:1081]
...
page2_1.update_layout(
    xaxis = dict(
        tickmode = 'array',
        tickvals = [i*60 for i in range(7,25)],
        ticktext = [f"{i}시" for i in range(7,25)]),
    yaxis = dict(
        tickmode = 'array',
...
taxi_driving_nm = (np.array(drive_tx) + np.array(empty_tx)).tolist()[60:1081]
taxi_driving_nm = taxi_driving_nm[::60]

taxi_driving_nm_fig = px.bar(x=[i for i in range(7,25)], y=taxi_driving_nm)
taxi_driving_nm_fig.update_layout(
    xaxis = dict(
        tickmode = 'array',
        tickvals = [i for i in range(7,25)],
        ticktext = [f"{round(i)}시" for i in range(7,25)],
        title=""),
    yaxis = dict(
        tickmode = 'array',
...
# 3-1-1-a
ps_start_inf = ps_start_inf.loc[(ps_start_inf["start_time"] >= 420) & (ps_start_inf["start_time"] <= 1441)]
...
ps_end_inf = ps_start_inf.loc[(ps_end_inf["end_time"] >= 420) & (ps_end_inf["end_time"] <= 1441)]
...
for i in trips:
    if i["vendor"] == 1:
        if (i["timestamps"][0] >= 420) & (i["timestamps"][-1] <= 1500):
...
build_tab_1 = [
        html.Div(
            children=[
                 #page1
                 html.H4(html.B("2022년 02월 14일 07~24시 시뮬레이션 결과"), style={"color": COLORS["text"]}),
                 html.Br(),
                 html.H4(html.B("1. 호출요청 및 호출실패"), style={"color": COLORS["text"]}),
                 html.Br(),
                 #page1-a
                 dbc.Alert([html.Li(html.B(f"전체 호출 수:  총 {len(passenger_locations)}건 | 전체 실패 호출 수:  총 {len(all_fail_data)}건 | 호출 실패율 : {round(len(all_fail_data)/len(passenger_locations)*100,2)}%"),
        ...
        html.Div(
            children=[
                 ...
                 dbc.Row([html.H4(html.B("2. 시간대별 배차 실패지점 위치"), style={'color': COLORS["text"], 'width': '50%', 'display': 'inline-block'}),
                          html.H4(html.B("3. 읍면동별 승객 대기시간 분포"), style={'color': COLORS["text"], 'width': '50%', 'display': 'inline-block'})]),
                 html.Br(),
